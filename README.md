...
3. **GNU Make**
   - *Windows:* Nên cài đặt qua [Chocolatey](https://chocolatey.org/) (`choco install make`) hoặc sử dụng Git Bash / WSL.
   - *macOS:* Đã có sẵn hoặc cài qua Homebrew (`brew install make`).
...
2. Tạo file môi trường `.env` từ file ví dụ:
   ...
   cp .env.example .env
   ```
   *Lưu ý:* Mở file `.env` vừa tạo và chỉnh sửa cấu hình kết nối database, redis, JWT secret, hoặc Gmail SMTP phục vụ việc gửi mã OTP thực tế (nếu cần).
...
```
*Lưu ý:* Sau đó chạy ứng dụng Go ở local bằng lệnh `make dev` hoặc `go run`.
...
*Kiểm tra trạng thái:* Chạy lệnh `docker ps` để đảm bảo 3 container (`postgres-smartwardrobe`, `redis-smartwardrobe`, `backend-smartwardrobe`) đang hoạt động bình thường.
...
make install-tools
```
*(Hoặc chạy thủ công các lệnh sau nếu không có Make)*:
```bash
...
| `make dev` | Chạy toàn bộ quy trình từ tidy, wire, swagger đến run | *(Tổ hợp các lệnh trên)* |
...
* Các API đã được phân nhóm một cách trực quan:
  * 🔐 **Auth:** Đăng ký, Đăng nhập, Xác thực OTP, Khôi phục mật khẩu.
  * 👤 **Me:** Quản lý thông tin cá nhân, Đổi mật khẩu tài khoản hiện tại.
