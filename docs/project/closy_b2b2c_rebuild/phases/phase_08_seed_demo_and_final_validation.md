...
- 1 offline brand_customer với `user_id NULL`.
```
...
- brand_customer_claims cho offline customer nếu demo claim/link.
```
...
Nếu phone chưa tồn tại -> tạo hoặc dùng lại brand_customer offline theo brand_id + phone_hash
...
```text
- brand_customers.user_id = NULL
- loyalty_accounts.user_id = NULL
- loyalty visible in brand portal
- không có users row mới được tạo từ phone offline
- offline customer chưa dùng app/chat đầy đủ
```
...
Link brand_customers.user_id và loyalty_accounts.user_id với current user
```

Expected:

```text
- brand_customers.user_id = current_user.id
- brand_customers.claimed_at set
- brand_customer_claims.consumed_at set
- loyalty_accounts.user_id = current_user.id
...
- [ ] Claim/link flow works with claim code/QR.
