...
1. **Frontend:** Người dùng chọn hoặc chụp ảnh quần áo. Frontend tải ảnh trực tiếp lên **Cloudinary** để lấy `ImageUrl` và `ImagePublicID`.
2. **API Request:** Frontend gửi HTTP POST tới `/api/v1/wardrobe/items/batch` với body chứa danh sách các ảnh (`ImageUrl`, `ImagePublicID`) và `CategoryID` (nếu chọn thủ công).
...
1. **API Request:** Người dùng nhấn "Lưu Outfit", Frontend gửi HTTP POST tới `/api/v1/wardrobe/outfits` chứa thông tin outfit (tên, các Item ID được chọn kèm toạ độ hiển thị).
...
1. Seller gửi HTTP POST tới `/api/v1/community/posts` với `PostType = "SALE"`, đính kèm danh sách các Wardrobe Item ID, giá tiền và thông tin liên hệ.
...
4. **Database Transaction:**
   - Tạo bài đăng `Post`.
   - Tạo các bản ghi `PostItem` với trạng thái `Status = "available"` và `TransferState = "none"`.
...
1. Buyer xem bài đăng và gửi yêu cầu mua thông qua HTTP POST tới `/api/v1/community/transfers/requests` kèm danh sách `PostItemIDs`.
2. Hệ thống kiểm tra:
...
1. Seller chọn một Buyer trong danh sách yêu cầu và gửi yêu cầu xác nhận bán qua HTTP POST tới `/api/v1/community/transfers/sold` kèm danh sách `PostItemIDs` và `BuyerID` được chọn.
...
1. Buyer gửi HTTP POST tới `/api/v1/community/transfers/accept` kèm danh sách `PostItemIDs`.
2. **Database Transaction:**
...
1. Buyer gửi HTTP POST tới `/api/v1/community/transfers/decline` kèm danh sách `PostItemIDs`.
2. **Database Transaction:**
...
1. **API Request:** User gửi HTTP POST tới `/api/v1/billing/purchase/direct` chứa `PlanSlug`, `ReturnUrl` và `CancelUrl`.
...
1. **Webhook Trigger:** Cổng thanh toán PayOS gửi POST request kèm chữ ký số và dữ liệu giao dịch về endpoint webhook `/api/v1/billing/webhook/payos`.
...
sequenceDiagram
    ...
    User->>API: Gọi API mua gói bằng ví (PlanSlug)
...
1. **API Request:** User gửi HTTP GET tới `/api/v1/community/posts?sort=hot` để lấy feed.
