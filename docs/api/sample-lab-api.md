...
### 1. Lấy danh sách sản phẩm (product) của thương hiệu (public, phân trang)
*   **Endpoint:** `GET /api/v1/brands/:brandId/items`
*   **Tác nhân (Actor):** Public (không yêu cầu xác thực).
*   **Đối tượng ảnh hưởng:** Đọc danh sách các sản phẩm thương mại `brand_items` có `itemType = product` và `status = active` của brand.
*   **Mô tả:** Trả về danh sách sản phẩm thương mại đang hoạt động, phân trang. Chỉ lọc `itemType = product`. Bộ lọc loại vật phẩm `itemType` và trạng thái `status` tham chiếu chi tiết tại [constants/brand.md:BrandItemType](constants/brand.md#11-phan-loai-vat-pham-trong-digital-sample-lab-branditemtype) và [constants/brand.md:BrandItemStatus](constants/brand.md#10-trang-thai-vat-pham-trong-digital-sample-lab-branditemstatus).
*   **Query Params:**
    *   `page` (optional): Trang hiện tại, mặc định `1`.
    *   `limit` (optional): Số lượng mỗi trang, mặc định `20`.
*   **Response:**
    *   `200 OK`: Trả về danh sách sản phẩm phân trang `BrandItemListRes`.

### 1b. Lấy danh sách mẫu thử (sample) của thương hiệu (yêu cầu sample_mix_access)
*   **Endpoint:** `GET /api/v1/brands/:brandId/items/samples`
*   **Tác nhân (Actor):** Khách hàng (Customer) đã đăng nhập.
*   **Đối tượng ảnh hưởng:** Đọc danh sách các mẫu thử `brand_items` có `itemType = sample` và `status = active` của brand.
*   **Mô tả:** Trả về danh sách mẫu thử kỹ thuật số phân trang. Yêu cầu user có quyền `sample_mix_access`. Nếu không đủ quyền trả về `403 Forbidden`.
*   **Query Params:**
    *   `page` (optional): Trang hiện tại, mặc định `1`.
    *   `limit` (optional): Số lượng mỗi trang, mặc định `20`.
*   **Error Response:**
...
### 2. Xem thông tin chi tiết một sản phẩm/mẫu thử đang active (product public, sample yêu cầu auth)
...
*   **Mô tả:** Trả về toàn bộ hồ sơ chi tiết của vật phẩm theo `itemId`. Backend tự xác định brand từ bản ghi `brand_items`, chỉ trả về item khi brand đang hoạt động và item ở trạng thái `active`. Nếu item có `itemType = product`, public hoàn toàn (không cần auth). Nếu item có `itemType = sample`, backend kiểm tra user đã đăng nhập và có quyền `sample_mix_access`; nếu không đủ quyền trả về `403 Forbidden`.
*   **Response:**
