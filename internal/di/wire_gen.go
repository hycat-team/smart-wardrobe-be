...
import (
	...
	contract2 "smart-wardrobe-be/internal/modules/brand/contract"
...
func InitializeApp(cfg *config.Config, l logger.Interface) (*bootstrap.App, func(), error) {
	...
	iBrandBenefitUseCase := usecase2.NewBrandBenefitUseCase(iBrandRepository, iBrandMemberRepository, iBrandCustomerRepository, iLoyaltyAccountRepository, iLoyaltyPointTransactionRepository, iLoyaltyPointLotRepository, iBrandBenefitRepository, iBenefitRedemptionRepository, iLoyaltyTierRepository, iUnitOfWork)
	...
	iBrandItemUseCase := usecase2.NewBrandItemUseCase(iBrandRepository, iBrandMemberRepository, iBrandCustomerRepository, iBrandItemRepository, iDigitalSampleResponseRepository, iUserRepository, iFashionContract, iMediaService, iBrandBenefitUseCase)
	iBrandContract := contract2.NewBrandContract(iBrandBenefitUseCase, iBrandItemUseCase)
	iOutfitUseCase := outfit.NewOutfitUseCase(cfg, l, iOutfitRepository, iWardrobeItemRepository, iSubscriptionUseCase, iBrandContract, iMediaService)
	...
	iBrandClaimUseCase := usecase2.NewBrandClaimUseCase(iBrandRepository, iBrandMemberRepository, iBrandCustomerRepository, iLoyaltyAccountRepository, iBrandCustomerClaimRepository, client, iUnitOfWork, cfg)
	brandLoyaltyHandler := handler4.NewBrandLoyaltyHandler(iBrandUseCase, iBrandLoyaltyUseCase, iBrandBenefitUseCase, iBrandClaimUseCase)
	...
	iBrandChatUseCase := usecase2.NewBrandChatUseCase(iBrandRepository, iBrandMemberRepository, iBrandCustomerRepository, iUserRepository, iBrandConversationRepository, iBrandConversationMessageRepository, iUnitOfWork)
	brandChatHandler := handler4.NewBrandChatHandler(iBrandChatUseCase)
	brandHandler := handler4.NewBrandHandler(brandPortalHandler, brandLoyaltyHandler, brandChatHandler)
	brandRouter := brand.NewRouter(brandHandler, authMiddleware)
	iOutfitRecommendationUseCase := recommendation.NewOutfitRecommendationUseCase(cfg, l, iWardrobeItemRepository, iaiService, iSubscriptionUseCase, iUserQuotaUseCase, iBrandContract, iUnitOfWork)
	...
	iWardrobeChatUseCase := chat.NewWardrobeChatUseCase(cfg, iConversationalContextRepository, iMessageRepository, iWardrobeItemRepository, iaiService, iSubscriptionUseCase, iUserQuotaUseCase, iBrandContract, iUnitOfWork)
	...
	iLoyaltyPointExpiryWorker := worker2.NewLoyaltyPointExpiryWorker(iBrandLoyaltyUseCase, l, cfg)
